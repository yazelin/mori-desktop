...
### 變更
- **VoiceInput starter 邊界補強** — USER-00~05 中文範本明確要求「只整理口述文字,不要回答內容」;即使使用者口述的是問句 / 命令 / 請求,也要輸出可貼給別人的文字或 prompt,避免 VoiceInput cleanup 被誤當成問答助手。
...
這版補完。重新讀 `docs/` 的 user 進去不會再看到「Windows / macOS paste-back 尚未接」這種 v0.3 era 過時敘述。
...
| `profile-examples.html` | 新增「v0.4.1+ 新增 starter」表格(12 個中文 + 12 個英文)、EN 對照版說明、Profiles「加入範本」UI 還原 / 換語系流程 |
| `mori-home.html` | `~/.mori/` 結構樹更新:agent / voice_input 都列 0..05、新增 `logs/`、`installed-apps.<platform>.json`、`runtime.json` 條目,`corrections.md` 改 `[auto]` 標 |
| `hotkeys.html` | Slot 0~5 都有 starter(過去寫 0+1)|
| `tokenizer-comparison.md` | 微 polish 文案(v0.4.1 加的,基本沒 stale)|

### 工程細節

- 沒動 `index.html` / `mori-home.html` 的 landing-style hero / feature highlight section — 那些是行銷頁,跟「feature manual」分工不同
- 沒動 `brand.html` / `character.html` / `design-book.html` / `tray.html` — 設計 / 美術 / character spec,跟 v0.4.0~v0.5.1 的功能變動關聯小
- 沒動 v0.4.0 加的 `LogsTab` / v0.5.0 加的 `installed_apps` 兩個觀測層元件的獨立深入文件 — 之後 v0.5.3+ 再開 `docs/observability.html` 之類專頁。現階段 troubleshooting + mori-home 兩處有提即可

### 留 v0.5.3+

- 觀測層獨立深入文件:`logs.html`(JSONL schema / kind 列表 / filter usage 教學)
- Installed apps catalog 獨立頁:`installed-apps.html`(usage signal 來源 / refresh 機制 / open_app 注入細節)
- Context anti-injection + corrections.md 機制深入頁:`prompt-engineering.html`(system prompt 結構 + redact + anti-injection 三層防護解析)
- **Phase B per-pipeline artifacts**(每次語音 pipeline 完整 I/O snapshot,Whisper fine-tune training data)

---

## v0.5.1 — STT corrections baseline + Context anti-injection(2026-05-15)

兩個 system prompt 層的補強:
...
新 const `DEFAULT_CORRECTIONS_MD` 走 hardcoded `&'static str`(~2.5KB),fresh install 第一次啟動就部署到 ~/.mori/。5 unit tests 鎖內容(`has_required_sections` / `has_attribution` / `user_section_exists` / `mori_specific_included` / `substantial_size >= 80 rules`)。檔案內 baseline 段標頭含致謝 line(內容來源透明)。
...
- **沒做 XML-like `<CONTEXT>` 結構**:XML tag 包 context(`<CURRENT_TIME>...</CURRENT_TIME>` 之類)對 LLM parsing 通常比 markdown 標題更穩定 — Mori 目前走 markdown,風險:大改 prompt 結構可能影響既有 LLM 行為,留 v0.5.2 評估
...
Phase C — Mori 在每個平台 scan 已裝的 desktop apps,寫進 `~/.mori/installed-apps.<platform>.json` cache,當 active profile 啟用 `open_app` skill 時把**最近 50 個常用 app 列表**塞進 LLM 看到的 tool description。LLM 不再憑「user 講 'SQL' 就猜成 SQL Server / SQLite / 其他」,直接 match 列表內的 display name。
...
- **Phase B per-pipeline artifacts**(每次語音 pipeline 完整 I/O snapshot,累積 Whisper fine-tune training data)
...
- **Phase B per-pipeline artifacts**(`~/.mori/recordings/<時戳>/` 完整 I/O snapshot)— 每次語音 pipeline 存 audio + STT request/response + LLM I/O + 最終輸出,給 Whisper fine-tune training data + 除錯用
...
3 個新範本(USER-03.LINE貼文 / USER-04.哄老婆開心 / USER-05.提示詞優化 + AGENT-05.聽我指令)。冪等:已有同名檔不覆蓋,使用者自己改過的內容保留。
...
設計重點:clipboard 注入是 prompt 增強的關鍵能力,但同條 path 也是 secret 外漏的最大攻擊面 — user 可能複製到 `gsk_xxx` / `sk-xxx` 後立刻跟 Mori 講話,整段 system prompt 就帶著 key 送 LLM provider。**v0.4 把 LLM API 這條路堵住**(redact 在 in-memory 階段做掉,token 永遠不離開本機);Phase B per-pipeline artifacts 寫磁碟前也會走同一條 redact path。
...
- 觀測層 3 層架構(留 v0.5+):**Phase A 已 ship**(JSONL log + LogsTab)、**Phase B** per-pipeline artifacts(`~/.mori/recordings/<時戳>/` 完整 I/O snapshot)、**Phase C** installed apps catalog(open_app skill 注入);per-turn provider badge in chat thread 也是觀測層後續延伸
