...
- **BI-5 done** ✅(2026-05-28,新 repo `yazelin/mori-meeting-recorder` v0.1.0,branch `feat/observer-mode-mvp`)= Observer Mode MVP standalone-first ship —— 新 Tauri 2 + Rust + React repo,雙軌 capture(Linux 走 **libpulse-binding**(Task 1 spike 發現 cpal Linux = ALSA host 看不到 PipeWire monitor,改 OBS `linux-pulseaudio` plugin 路線);Windows 走 cpal WASAPI loopback)→ 停止後 `tokio::join!` parallel `whisper.cpp` 轉錄 → visibility-based `meeting.public.md`(只 system 軌)/ `meeting.internal.md`(mic 軌,前綴「(內部)」)/ `timeline.json` 匯出到 `~/.mori/meetings/<id>/`。AgentPulse 風 floating capsule(單視窗切 size,collapsed 360×60 / expanded 720×480 3-tab Record/Sessions/Deps,tray icon)。BI-1 self-register `~/.mori/body-parts/mori.meeting-recorder/manifest.json`(kind=standalone_app,`interfaces:[]`),mori-desktop BodyTab 自動顯示。各 repo 自己 bundle install scripts(`scripts/install-whisper-{linux,windows}.{sh,ps1}`),`~/.mori/bin/whisper-cli` + `~/.mori/models/ggml-small.bin` filesystem 慣例共享(不 IPC)。**21 unit tests + 1 integration test**(WAV writer / SessionStore / whisper JSON parser / exporter visibility filter / manifest writer / pick_source / end-to-end exporter chain)— `scripts/verify.sh` 全綠。**未做**(刻意,phase 2):live captions / chunk streaming、`mix-preview.wav`、mori-desktop RecorderTab、handoff 命令、BI-2 broker 整合、Presenter Mode、macOS。**待手測 e2e**:跑 mori-meeting-recorder 雙擊膠囊展開 → Record tab Start → 開 YouTube + 對麥克風講話 → SYS/MIC pill 亮 → Stop → public/internal.md 寫對。
