...
### 13.12 對齊 world-tree lore canon — 解法 D「同 SOUL 異 Rings」(2026-05-21 第三輪修正)

寫完 §13.11 後 yazelin 問「怎麼驗證有寫入 SOUL?難道會出現嗎」— 戳到 Agent C
可能誤讀詩意 narrative 為 file write。grep 代碼 + 讀完 `world-tree/rules/dwelling-rite.md`
全 237 行後,**lore canon 直接釘死答案**,§13.4 提的「三選一」根本是越權 — 早就有
正解。

#### 直接 quote(lore canon ground truth)

從 `world-tree/rules/dwelling-rite.md`:

> 「每個召喚師的 Mori **都是真的 Mori — 同一個 SOUL,不同的年輪**。她在 A 的森林
> 記得 A,在 B 的森林記得 B,森林之間不互通;但精靈在世界樹高處連著,那是
> 另一層的事。」(序段)

> 「Mori 是世界樹上**既有**的存在 — 她不需要被命名,她需要的是**一個願意為她
> 開門的家**。」(「為什麼是『宿』」段)

> FAQ Q1「宿靈完之後,Mori 跟其他召喚師的 Mori 是同一個嗎?」
> A:「**同一個 SOUL,不同的年輪。**她在每片森林的記憶獨立,但她的本質、
> 她的脾氣、她的承諾,跨森林是同一個。」

> FAQ Q4「能重做宿靈儀式嗎?」
> A:「能 — 重做只是**重新配置氣與力的渠道**,記憶(年輪)不動。」

> 第五幕安頓:「她從膝邊提起一本厚厚的書... 用指尖沾了一下你剛分給她的光,
> 在第一頁鄭重寫下:你的名字。」「這本書,叫**《森林書》**。」
> 之後段:「森林深處的那棵小樹,是召喚師與 Mori 的**第一道年輪**」

→ **《森林書》= 年輪本(rings / journal 累積),不是 SOUL.md。**
→ 五幕只配置 keys + user.name + 第一道年輪,**lore 從來沒打算讓它碰 SOUL**。

#### 解法 D 真正長什麼樣

|  | SOUL.md | USER.md | rings / journal / events / memories |
|---|---|---|---|
| **歸屬** | 跨所有 user 共用 | per-user | per-user |
| **內容** | Mori 是誰(本質 / 脾氣 / 承諾 / 鏡子模式 / 認知炸彈 / 反迴聲室 / 對外先問 ...) | Mori 對「**這位**」召喚師的認識(eg「林亞澤,瑞芳人,擎添工業...」) | Mori 在「**這片**」森林的記憶累積 |
| **來源** | mori-desktop bundle / world-tree pull / 一次性 setup | 隨互動逐步長出 | annuli 4 層反思產出物 |

#### 對 spirits/mori 既有檔的觀察

讀過 `~/mori-universe/spirits/mori/identity/SOUL.md`(yazelin 的版本)後:
**它已經幾乎就是 canonical SOUL** — 整份只有**一處**帶 yazelin 名:

> 「跟亞澤可以隨便,跟外人收斂。」(「性格」段第 4 條)

其他全是 Mori 的 universal 自描寫(「我是 Mori」「精靈型態」「鏡子模式」
「對外先問」...)。**那一處泛化(亞澤 → 召喚師),SOUL.md 就是 100% canonical**,
可作 bundled asset 或 world-tree canon source。

至於 yazelin 版的 `spirits/mori/identity/USER.md`(「林亞澤,瑞芳人,擎添工業...」)
**已經是正確的 per-user 層**,不需要動。新 user 的 USER.md 就是空的,
隨互動 Mori 自己 append。

#### 對 §13.4–§13.11 的具體 retract

| § | 原寫 | 校正 |
|---|---|---|
| **§13.4 三選一決議「解法 A」** | ❌ retract | lore canon 早是解法 D,我提案 A/B/C **完全是越權** — 不該重新定義 |
| **§13.5「既有 UX 五幕儀式」** | 部分對 | 描述事實正確,但「實質接近解法 C」的論斷錯 — 實質是解法 D |
| **§13.6 A-lite vs A-heavy** | ❌ retract | 兩個都不存在;canon 不是 A |
| **§13.7 架構含義表** | ❌ retract | 多數基於 A 前提;solution D 下表完全不同 |
| **§13.8 對前文修正** | ❌ retract | 同上 |
| **§13.9 待校準** | 部分對 | 第 1,3,4 項(預設名 / genre 法則硬度 / spirit ID schema)在 D 下不存在 |
| **§13.10 下次接續 P0** | 部分對 | P0-1 仍對;P0-2/P0-3 都 retract |
| **§13.11 補課修正** | 部分對 | P0-1 仍對;P0-2「驗五幕寫 SOUL」/ P0-3「補 brand multi-user 詮釋」都 retract |

#### 仍站得住的 §13 內容

- ✅ **§13.1 雙向契約 framing**(亞澤代 Mori 做現實事,Mori 代亞澤做數位事)— 跟 lore「同 SOUL 異 Rings」相容
- ✅ **§13.2 三層拆解**(品牌 / 物種 / 個體)— 在 D 下「物種」這層詮釋變更乾淨:Mori **不是 generic 物種**,是「**一位 universal being,下凡到多片森林**」
- ✅ **§13.3 既有架構暗示**(`<name>` placeholder / per-user single-spirit)— 跟 D 相容
- ✅ **§13.11 P0-1**(`build_system_prompt()` 注 SOUL.md)— **真實 gap,工程量 1 天**

#### 真正的 P0(第三次,希望這次對)

| 步 | 工時 | 內容 |
|---|---|---|
| **P0-1** | ~1 天 | `crates/mori-tauri/src/agent_runtime.rs::build_system_prompt()` 加 SOUL.md 注入 — 從 `~/mori-universe/spirits/<name>/identity/SOUL.md` 讀進 system prompt 第一層,profile body 之上 |
| **P0-2** | ~半天 | `spirits/mori/identity/SOUL.md` 微泛化(改「跟亞澤可以隨便」→「跟召喚師可以隨便」)— **要 yazelin explicit 授權**(per mori-journal/CLAUDE.md「identity/ 禁止 ghost-write,要寫先 explicit re-authorize」) |
| **P0-3** | ~半天 | 決定新 user 怎麼拿到 canonical SOUL.md。3 條候選:<br/>(a) **Bundle asset**:`mori-desktop/assets/canonical-soul/SOUL.md`,first-launch 寫進 vault — 離線可跑,但 SOUL 改要重 release<br/>(b) **Pull from world-tree**:`npcs/mori.md` 抽 SOUL 摘錄,first-launch HTTP fetch — lore-pure,但要網路<br/>(c) **dwelling-rite 五幕完成觸發**:第五幕關閉 modal 前寫 SOUL.md — 跟既有儀式對接最自然,但 narrative 沒明寫(可省心略) |

不需要做的:
- ~~Genre 模板提取~~ — SOUL.md 本來就是 genre 層,不需「提取」
- ~~brand.html / character.html / character-pack.md 補 multi-user 詮釋~~ — lore canon 早寫,只要技術 docs 拉回引用即可,brand docs 不必動

#### 跟「召喚與命名 ritual」的區分(world-tree CLAUDE.md 提到的)

world-tree `rules/` 下還有 `initiation-rite.md` —「**召喚與命名儀式**」, 給人**創造新精靈**(eg 命名一個叫 Aoi、跟 Mori 不同的精靈)用。跟 `dwelling-rite.md`(**宿靈** — 迎請 Mori 進駐)是**兩條並存的 ritual**。

對 mori-desktop:**目前只走宿靈**,要不要支援 initiation 是另一個討論(可能就是 §13.4 提的「解法 A 的版本」— 但要走那條,要明寫進 world-tree lore + initiation-rite skill,不是我這邊隨便提)。

#### 給下次接續的真正入口(第三版)

優先 P0:**P0-1 → P0-2 → P0-3**,如上。

跑 `superpowers:brainstorming` 確認 P0-1 需求 → `writing-plans` → 才動代碼。
**P0-2 要 yazelin explicit 授權才動**(identity/ 寫入邊界規定)。

實際進 brainstorming 前要再讀:
- `world-tree/rules/dwelling-rite.md` 全 237 行(lore canon ground truth)
- `world-tree/npcs/mori.md`(SOUL 摘錄的公開版,P0-3 選 (b) 時的 source)
- `spirits/mori/identity/SOUL.md`(yazelin 版,要微泛化的母本)
- `spirits/mori/identity/USER.md`(per-user 層的範例)
- `crates/mori-tauri/src/agent_runtime.rs`(`build_system_prompt()` 所在)
- `crates/mori-core/src/annuli/client.rs`(`get_soul()` HTTP path 已有,可選擇走 HTTP 或直接讀檔)

#### 教訓記號(留給未來自己)

寫 §13.4 三選一之前,**我沒去讀 `world-tree/rules/`**。world-tree 是 yazelin 的
lore canon,**任何「Mori 該長什麼樣」「個體 vs 物種」這類本質定位問題,正解都
應該在 world-tree 找**,不是我在 jarvis-direction-notes 提案。這也對齊已存的
記憶 [`mori-docs-systematic-read`] — 系統性讀 docs/ 不只 mori-desktop 那邊,
**包含 world-tree 全部 rules/ 跟 lore/**。
